...
El método propuesto implementa un pipeline de experimentación de cinco etapas diseñado para garantizar la reproducibilidad y la comparabilidad justa entre configuraciones. Las etapas comprenden: (1) preparación de datos, (2) aumento de datos, (3) entrenamiento de modelos, (4) evaluación y (5) análisis de resultados. Cada etapa se ejecuta de forma independiente mediante un sistema de caché por pasos que almacena los resultados intermedios en disco, permitiendo la re-ejecución parcial sin reprocesar etapas completas. La caché se basa en existencia: tras cualquier cambio de código o configuración el pipeline se re-ejecuta completo con ejecución forzada, y cada acierto de caché se registra con la huella de la configuración solicitante para auditar reutilizaciones obsoletas. La configuración de toda la tubería se gestiona mediante modelos Pydantic tipados que garantizan la validación de parámetros en tiempo de compilación @colvin2024pydantic. La interfaz de línea de comandos permite la personalización de cualquier parámetro anidado mediante notación de puntos, facilitando la experimentación reproducible. Todos los generadores aleatorios (Python, NumPy, PyTorch) se siembran desde una semilla maestra con identificadores de flujo fijos por etapa; cudnn benchmarking y TF32 permanecen activos, por lo que la pequeña variación residual entre corridas se declara en lugar de eliminarse.
...
BoxAug con LibCom utiliza el mismo procedimiento de recorte y pegado que BoxAug estándar, pero reemplaza la inserción directa por composición profunda mediante la librería LibCom @niu2021making. Las operaciones de borde emplean modos de armonización neuronal que eliminan las discontinuidades visuales en los márgenes entre el fondo y la imagen superpuesta. Las transformaciones geométricas se limitan a jitter de escala y volteo horizontal/vertical, excluyendo transformaciones de ruido a nivel de píxel. El modo de fusión por defecto es composición Poisson; cada pegado registra su modo efectivo en un manifiesto de corrida para visibilizar corridas degradadas.

Cascade R-CNN representa al paradigma de dos etapas. Posee una arquitectura convolucional basada en ResNet-50 con Feature Pyramid Network (FPN) @cai2018cascade. Se entrena durante 24 épocas siguiendo el programa estándar 2x de MMDetection con optimizador AdamW con learning rate de 0.0001 y weight decay de 0.0001, batch size de 8 y resolución de entrada de 960 por 384 píxeles preservando el aspecto. La evaluación de validación se ejecuta en cada época utilizando la métrica COCO bbox con early stopping de paciencia 15 y mejora mínima 0.005, y se selecciona el checkpoint con mayor mAP bbox. Esta configuración de entrenamiento relativamente conservadora con learning rate bajo busca explotar las capacidades de refinamiento multietapa de la arquitectura sin riesgo de divergencia @cai2018cascade.

YOLO26 representa al paradigma de una etapa. Posee una arquitectura convolucional libre de anclajes con diseño de cabeza dual que elimina la etapa NMS @jocher2026ultralyticsyolo26unifiedrealtime. Se configura con la variante yolo26m, resolución de entrenamiento rectangular de 960 por 384 píxeles, batch size de 16 y learning rate inicial de 0.01. El entrenamiento utiliza early stopping con la paciencia por defecto de Ultralytics de 100 épocas y guarda checkpoints cada 5 épocas. El número máximo de épocas es 100.

RF-DETR representa al paradigma basado en transformers. Posee una arquitectura con búsqueda de arquitectura neuronal @robinson2026rfdetrneuralarchitecturesearch. Se utiliza la variante rfdetr-m con resolución de 512 por 512 píxeles, batch size de 8, learning rate de 0.0001 y weight decay de 1e-4. El entrenamiento incluye warmup de 5 épocas, early stopping con paciencia de 15 según la guía oficial, y gradient checkpointing para reducir el consumo de memoria. El número máximo de épocas es 50.
